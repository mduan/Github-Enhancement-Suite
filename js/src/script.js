...
function _parseLineNumberCell($cell) {
  return new LineNum({
    id: $cell.attr('id'),
...
function _parseLineIndex($row, rowIndex) {
  return parseInt(
...
function parseFileDiff($fileDiff) {
  ...
  $fileDiff.find('tr').each(function() {
    ...
    if ($row.hasClass('file-diff-line')) {
      ...
      var row;
      var codeData = _parseCodeCell($cells.eq(2));
      var rowText = codeData.text;
      var commentUrl = codeData.commentUrl;
      var deletedLineNum = _parseLineNumberCell($cells.eq(0));
      var insertedLineNum = _parseLineNumberCell($cells.eq(1));
